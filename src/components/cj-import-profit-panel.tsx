...
import { estimateCJImportProfit } from "@/lib/cj-dropshipping.functions";
...
export function CJImportProfitPanel({ productId, productName, sellOverride, mrpOverride, onApply }: Props) {
  const estimateFn = useAuthedServerFn(estimateCJImportProfit);
  const [estimate, setEstimate] = useState<(CJProfitEstimate & { preview?: { profit: number; marginPercent: number; isLoss: boolean } }) | null>(null);
  ...
  const load = async (sellPreview?: number) => {
    ...
    try {
      ...
      setEstimate(res);
      if (!logisticName && res.selectedShipping?.logisticName) {
  ...
  return (
    <div className="rounded-xl border-2 border-primary/25 bg-primary/5 p-4 md:p-5 space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase text-primary flex items-center gap-1.5">
            <Calculator className="h-4 w-4" /> Profit Calculator — import se pehle dekho
          </p>
          <p className="font-bold text-sm mt-1 line-clamp-2">{productName}</p>
          <p className="text-xs text-muted-foreground mt-0.5">CJ product cost + India shipping = aapka asli kharcha</p>
        </div>
      ...
      {loading && !estimate ? (
        <p className="text-sm text-muted-foreground">Calculating CJ costs…</p>
      ) : estimate ? (
        <>
          {estimate.highShippingWarning && (
            <div className="flex gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 dark:bg-amber-950/30 dark:border-amber-800 dark:text-amber-100 text-sm">
              ...
              <p>
                ...
                product cost (${estimate.productCostUsd.toFixed(2)}) se zyada hai. Selling price carefully set karo.
              </p>
            </div>
          )}
          ...
          <div className="grid sm:grid-cols-3 gap-3 text-sm">
            ...
            <div className="rounded-lg bg-card border p-3">
              ...
              <p className="text-xs text-muted-foreground">{estimate.selectedShipping.logisticName} · {estimate.selectedShipping.logisticAging} days</p>
            </div>
          ...
          <div>
            <p className="text-xs font-bold uppercase text-muted-foreground mb-2">Recommended Selling Price (profit margin)</p>
          ...
          <div className="flex flex-wrap gap-3 items-end border-t pt-4">
            <div>
              <Label className="text-xs">Try Selling Price ₹</Label>
              <Input
                type="number"
                value={trySell}
        ...
        <p className="text-sm text-destructive">Could not load CJ shipping estimate. Retry or check CJ API.</p>
      )}
    </div>
  );
}
