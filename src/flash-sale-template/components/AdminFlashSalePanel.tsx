...
import { profitFromSelling } from "../lib/flash-sale-pricing";
import { cn as defaultCn } from "../lib/cn";
import type {
...
export type AdminFlashSalePanelProps = {
  categories: { slug: string; name: string }[];
  formatPrice: (amount: number) => string;
  defaultPincode?: string;
  onLoad: () => Promise<FlashSaleAdminLoadResult>;
  onSave: (payload: FlashSaleSavePayload) => Promise<{ productCount: number }>;
  onEstimateLandedCost?: (
    product: FlashSaleAdminProduct,
    options: { pincode?: string; logisticName?: string; sellPreview?: number },
  ) => Promise<LandedCostEstimate | null>;
  ProfitPanel?: ComponentType<FlashSaleProfitPanelProps>;
  onToast?: (message: string, type?: "success" | "error") => void;
  onConfirmLoss?: (lossCount: number) => boolean | Promise<boolean>;
  ...
  Label: ComponentType<LabelHTMLAttributes<HTMLLabelElement>>;
  Button: ComponentType<ButtonHTMLAttributes<HTMLButtonElement> & { variant?: string; size?: string }>;
  cn?: (...parts: (string | false | null | undefined)[]) => string;
  icons?: {
    Save?: ComponentType<{ className?: string }>;
    Search?: ComponentType<{ className?: string }>;
    Zap?: ComponentType<{ className?: string }>;
    RefreshCw?: ComponentType<{ className?: string }>;
    CheckCircle2?: ComponentType<{ className?: string }>;
    AlertTriangle?: ComponentType<{ className?: string }>;
  };
};

export function AdminFlashSalePanel({
  categories,
  formatPrice,
  ...
  const [landed, setLanded] = useState<Record<string, LandedCostEstimate>>({});
  ...
  useEffect(() => {
    load().catch((e) => toast(e instanceof Error ? e.message : "Load failed", "error"));
  }, [load]);

  const loadLandedCost = useCallback(
    async (p: FlashSaleAdminProduct, sellPreview?: number) => {
      if (!onEstimateLandedCost) return null;
      if (!p.cj_cost_usd && !p.cj_product_id) return null;
      try {
        const est = await onEstimateLandedCost(p, {
          pincode: pincode || undefined,
          logisticName: logisticName || undefined,
          sellPreview: sellPreview ?? p.salePrice,
        });
        if (est) setLanded((prev) => ({ ...prev, [p.id]: est }));
        return est;
      } catch {
        return null;
      }
    },
    [onEstimateLandedCost, pincode, logisticName],
  );

  const refreshAllCosts = async () => {
    if (!onEstimateLandedCost) {
      toast("Landed cost estimator not configured", "error");
      return;
    }
    const targets = catalog.filter((p) => p.inFlashSale && (p.cj_cost_usd || p.cj_product_id));
    if (!targets.length) {
      toast("No CJ-linked products in flash sale", "error");
      return;
    }
    ...
    toast(`Landed cost loaded for ${ok}/${targets.length} products`);
  };
  ...
  const filtered = useMemo(() => {
    ...
    return included.filter((p) => p.name.toLowerCase().includes(q) || p.category.includes(q));
  }, [included, filter]);
  ...
  const save = async () => {
    if (!settings) return;
    if (summary.inLoss > 0) {
      const ok = onConfirmLoss
        ? await onConfirmLoss(summary.inLoss)
        : window.confirm(
            `${summary.inLoss} product(s) are at a LOSS at flash sale price. Save anyway?`,
          );
      if (!ok) return;
    }
    setBusy(true);
    try {
      ...
      if (settings.enabled && items.length === 0) {
        toast("Enable at least one category or product for Flash Sale", "error");
        setBusy(false);
        return;
      }
      ...
      toast(`Flash Sale saved — ${res.productCount} products synced`);
      await load();
    } catch (e) {
  ...
  if (!settings) {
    return <p className="text-sm text-muted-foreground">Loading flash sale settings…</p>;
  }
    ...
    <div className="space-y-6">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <div className="rounded-xl border bg-success/10 border-success/30 p-4">
            <p className="text-xs font-bold uppercase text-success flex items-center gap-1">
              {CheckIcon && <CheckIcon className="h-4 w-4" />} In Profit
            </p>
          ...
          <div className="rounded-xl border bg-destructive/10 border-destructive/30 p-4">
            <p className="text-xs font-bold uppercase text-destructive flex items-center gap-1">
              {AlertIcon && <AlertIcon className="h-4 w-4" />} In Loss
            </p>
          ...
          <div className="rounded-xl border bg-muted/50 p-4">
            ...
            {onEstimateLandedCost && (
              ...
              >
                {RefreshIcon && (
                  <RefreshIcon className={cn("h-3.5 w-3.5 mr-1", loadingCosts && "animate-spin")} />
                )}
                Load all CJ costs
              </Button>
            )}
      ...
      <div className="grid lg:grid-cols-[1fr_300px] gap-6">
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="font-bold flex items-center gap-2">
                  {ZapIcon && <ZapIcon className="h-4 w-4 text-primary" />} Flash Sale Master
                </h2>
                <p className="text-xs text-muted-foreground mt-1">OFF = homepage deals + /deals hidden</p>
              </div>
              <Switch
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              ...
              <div className="relative w-full sm:w-64">
                ...
                <Input
                  ...
                  className={SearchIcon ? "pl-9" : undefined}
                />
              </div>
            </div>
              ...
              <table className="w-full text-sm min-w-[900px]">
                ...
                <tbody>
                  {filtered.map((p) => {
                    ...
                    const rowProfit =
                      totalCost != null ? flashSaleLandedProfit(totalCost, p.salePrice) : null;
                    return (
                      <tr
                        key={p.id}
                        className={cn(
                      ...
                      >
                        ...
                        <td className="p-2">
                          {totalCost != null ? (
                            <span className="font-semibold text-primary">{formatPrice(totalCost)}</span>
                          ) : onEstimateLandedCost ? (
                            <button
                              type="button"
                        ...
                        <td className="p-2">{formatPrice(p.base_price)}</td>
                        <td className="p-2" onClick={(e) => e.stopPropagation()}>
                          <Input
                            type="number"
                            min={1}
                            ...
                            onChange={(e) =>
                              updateProduct(p.id, { salePrice: Number(e.target.value) || 0 })
                            }
                          />
                        </td>
                        <td className="p-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        <td className="p-2">
                          {rowProfit && rowProfit.profit != null ? (
                            ...
                            >
                              {rowProfit.isLoss ? "−" : "+"}
                              {formatPrice(Math.abs(rowProfit.profit))} ({rowProfit.marginPct}%)
                            </span>
                          ) : (
                            <span className="text-xs text-muted-foreground">
                              {onEstimateLandedCost ? "Load cost" : "—"}
                            </span>
                          )}
                        </td>
                      </tr>
          ...
          {selected && selected.inFlashSale && ProfitPanel && (
            <ProfitPanel
              key={selected.id}
              ...
              onApply={(sell, mrp) => {
                ...
                toast(`Sale ${formatPrice(sell)} · MRP ${formatPrice(mrp)} applied`);
              }}
            />
          )}
        </div>
        ...
        <aside className="space-y-4 h-fit lg:sticky lg:top-24">
          <div className="bg-card border rounded-xl p-5 space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Load all CJ costs first, then apply margin to all included products (landed cost).
            </p>
            <div>
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (!applied) toast("Load CJ costs first", "error");
                else toast(`${margin}% margin applied to ${applied} products`);
              }}
            >
          ...
          <div className="bg-muted/40 border rounded-xl p-4 text-xs text-muted-foreground space-y-2">
            <p>
              <strong className="text-foreground">Landed cost</strong> = product cost + shipping → INR.
            </p>
...
export type { AdminFlashSalePanelProps as FlashSaleAdminPanelProps };
